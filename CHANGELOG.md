...
- **OLED Care idle timer** — Set a number of minutes of inactivity before OLED Care activates automatically. Any input (keyboard, mouse, controller) turns it back off. Found in the Screen section. Saved to the shared presets file.
...
- **Fixed 1% low showing higher than max FPS** — Could happen during FG transitions or when loading screen frame times contaminated the rolling window. The 1% low window now clears immediately when FG toggles.
