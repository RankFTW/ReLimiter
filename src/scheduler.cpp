...
static void OnMarker_VRR(uint64_t frameID, int64_t now) {
    ...
    // ── Adaptive smoothing: P99-based interval extension (DX12+Reflex only) ──
    // Tracks the GPU's actual render time (gpuActiveRenderTimeUs from Reflex
    // GetLatency) and computes an offset that extends effective_interval so
    // 99% of frames complete within it.
    //
    // gpuActiveRenderTimeUs is the driver-measured GPU shader execution time
    ...
    if (g_config.adaptive_smoothing &&
        ...
        // for the real frame, excluding idle bubbles. This is the true cost
        // of rendering one frame. aiFrameTimeUs is NOT added — FG interpolation
        // runs as a separate pass and doesn't stack with the real frame's
        // render time.
