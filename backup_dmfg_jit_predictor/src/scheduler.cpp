...
void OnMarker(uint64_t frameID, int64_t now) {
    ...
    if (IsDmfgActive()) {
        ...
        if (output_cap > 0) {
            ...
            // Multiplier: use g_fg_actual_multiplier but hold the value stable.
            // numFramesActuallyPresented is a per-GetState-call delta, not a fixed
            // multiplier. It fluctuates as our pacing changes how many presents land
            // between GetState calls. We hold the last value >= 2 and only update
            // when the new value is also >= 2, filtering out the transient 1s and 0s.
            int raw_mult = g_fg_actual_multiplier.load(std::memory_order_relaxed);
            static int s_held_mult = 0;
            if (raw_mult >= 2) {
                s_held_mult = raw_mult;
            }
            int mult = s_held_mult;
            if (mult < 2)
                mult = ComputeFGDivisorRaw();  // fallback: Streamline hints
            if (mult < 2)
